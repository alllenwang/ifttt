...
<% if (username == null)
	...
	taskname = (String)request.getParameter("change[taskname]");
	if (taskname == null)
		response.sendRedirect("ManageTasks.jsp");
	tasktype = (String)request.getParameter("change[tasktype]");
	if (tasktype.compareTo("WeiboWeibo") != 0)
		response.sendRedirect("ManageTasks.jsp");
	%>
								...
								<div class="content">
									<div class="account-group js-mini-current-user"
										...
										<img class="avatar size32" src="./IMAGE/default_avatar.png"
											...
											<%= username %>
										</b> <small class="metadata">创建您自己的任务</small>
									</div>
								</div>
							</a>
				...
				<div role="main" aria-labelledby="content-main-heading"
					...
					<div class="content-inner no-stream-end">
						<form id="profile-form" class="form-horizontal" method="POST"
							...
							<input type="hidden" id="task[type]" name="task[type]"
								value="WeiboWeibo">
							<hr>
							<fieldset class="control-group">
								<label class="control-strong-label" for="user_location">检查微博</label>
							</fieldset>
							<fieldset class="control-group">
								<label class="control-label" for="user_location">微博账号</label>
								<div class="controls">
									<input id="this[weiboid]" name="this[weiboid]" type="text"
										placeholder="ifttt@ifttt.com">
								</div>
							</fieldset>
							<fieldset class="control-group">
								<label class="control-label" for="user_location">微博密码</label>
								<div class="controls">
									<input id="this[weibopassword]" name="this[weibopassword]"
										type="password">
								</div>
							</fieldset>
							<fieldset class="control-group">
								<label class="control-label" for="user_location">微博包含字段</label>
								<div class="controls">
									<input id="this[weibocontain]" name="this[weibocontain]"
										type="text"
										value="<%= TableTaskWeiboWeibo.containInfo(taskname) %>">
								</div>
							</fieldset>
							<hr>
							<fieldset class="control-group">
								<label class="control-strong-label" for="user_location">发送微博</label>
							...
							<fieldset class="control-group">
								<label class="control-label" for="user_location">微博账号</label>
								<div class="controls">
									<input id="that[weiboid]" name="that[weiboid]" type="text"
										placeholder="ifttt@ifttt.com"
										value="<%= TableTaskWeiboWeibo.weiboID(taskname) %>">
								</div>
							</fieldset>
							<fieldset class="control-group">
								<label class="control-label" for="user_location">微博密码</label>
								<div class="controls">
									<input id="that[weibopassword]" name="that[weibopassword]">
								</div>
							</fieldset>

							<fieldset class="control-group">
								<label class="control-label" for="user_description">内容</label>
								<div id="user_description_box" class="controls">
									<textarea class="input-xlarge" id="that[weibocontent]"
										name="that[weibocontent]"><%= TableTaskWeiboWeibo.text(taskname) %></textarea>
									<p class="bio-label">
										内容不得多于<strong>140</strong>个字。
									</p>
								</div>
							</fieldset>
