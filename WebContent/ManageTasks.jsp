...
<%! TaskInfoList tasklist = null; 
	String[] typelist = null;
	int listsize = 0; 
%>

<% tasklist = new TaskInfoList(username);
	listsize = tasklist.TaskList.size();
	typelist = new String[listsize];
%>
...
<link rel="stylesheet" href="./CUSTOMED/ManageTasks.css" type="text/css"> 
							...
							<a href="./UserHome.jsp"
								...
								<div class="content">
									<div class="account-group js-mini-current-user"
										...
										<small class="metadata">null</small>
									</div>
								</div>
							</a>
					...
					<div role="navigation" class="module">
						<ul class="js-nav-links">
							<li class=""><a class="list-link js-nav"
								href="./ChooseTaskType.jsp">新建任务 <i
									class="chev-right"></i>
							</a></li>
							<li class="active"><a class="list-link js-nav"
								href="./ManageTasks.jsp">管理任务<i
									class="chev-right"></i>
							</a></li>
							<li class=""><a class="list-link js-nav" 
				...
				<div class="content-main js-content-main breakable">
					...
					<div class="content-header js-content-header">
						<div class="header-inner">
							<h2>管理
							<%= (String)(session.getAttribute("username")) %>的任务</h2>
							<div class="example-tweet-info subheader" style="">
								null</div>
						</div>
					</div>
...
<% for (int i = 0; i < listsize; i ++) { 
		typelist[i] = tasklist.TaskList.get(i).taskType; %>
						<div class="unfocusable-stream-item stream-item" style="">
							<div
								class="tweet js-unclickable-tweet unclickable-tweet has-cards with-social-proof">

									<div class="content">
										<div class="stream-item-header">
											<span class="account-group"> <img class="avatar"
												src="" alt=""> <strong class="fullname"> <%=tasklist.TaskList.get(i).taskName%></strong>

											</span> <small class="time"> <%=tasklist.TaskList.get(i).createDay + " "
						+ tasklist.TaskList.get(i).createTime%>
											</small>
										</div>
										<% if ( Task.getState(tasklist.TaskList.get(i).taskName) == 1 ) { %>
											<p>任务状态：正在运行</p>
											<% } else if ( Task.getState(tasklist.TaskList.get(i).taskName) == 2 ) { %>
											<p>任务状态：已暂停</p>
											<% } else if ( Task.getState(tasklist.TaskList.get(i).taskName) == 3 ) { %>
												<p>任务状态：已完成</p>
											<% } else if ( Task.getState(tasklist.TaskList.get(i).taskName) == -1 ) { %>
											<p>任务状态：任务不存在，系统错误</p>
										<% } %>			
										<div class="stream-item-footer">

											<div class="context">null</div>
										</div>
									</div>
...
<!-- ----------------------------------- start task -------------------------------------- -->		
								<form action="./StartTask.jsp" method="POST">
									<input type="hidden" name="start[taskname]" value="<%=tasklist.TaskList.get(i).taskName%>">
...
<!-- ----------------------------------- pause task -------------------------------------- -->		
								<form action="./StartTask.jsp" method="POST">
									...
									<button class="btn" type="submit">暂停</button>
								</form>								
<!-- ----------------------------------- change task -------------------------------------- -->		
								<form action="./ChangeTask<%=typelist[i]%>.jsp" method="POST">
									<input type="hidden" name="change[taskname]" value="<%=tasklist.TaskList.get(i).taskName%>">
									<input type="hidden" name="change[tasktype]" value="<%=typelist[i]%>">
									<button class="btn" type="submit">修改</button>
								</form>
<!-- ----------------------------------- delete task -------------------------------------- -->									
								<form action="./DeleteTask.jsp" method="POST">
									<input type="hidden" name="delete[taskname]" value="<%=tasklist.TaskList.get(i).taskName%>">
									<input type="hidden" name="delete[taskType]" value="<%=typelist[i]%>">
